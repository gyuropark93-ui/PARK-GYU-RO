...
function GridBlock({ data }: { data: GridBlockData }) {
  const cols = data.urls.length <= 2 ? data.urls.length : data.urls.length <= 4 ? 2 : 3;
  return (
    <div className={`grid gap-2 grid-cols-${cols}`} style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
      {data.urls.map((url, idx) => (
        <img key={idx} src={url} alt="" className="w-full h-auto object-cover" />
      ))}
    </div>
  );
}
